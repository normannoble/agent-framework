...
3. Always open a **fresh pane** for the exchange, even if the target is already live somewhere (a live session belongs to {{PRINCIPAL}}; a peer request never lands in it): `herdr pane split --current --direction <right|down> --cwd "<root>" --no-focus` → new pane id; `herdr pane rename <id> "<Target> - Peer"`; `herdr agent start <target>-peer --kind claude --pane <id>`; `herdr agent prompt <target>-peer "/agents:start <target> peer agents/<Target>/peer/<file>" --wait --timeout 600000`.
