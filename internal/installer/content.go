...
Invoke with ` + "`/agents:start <name>`" + `. List with ` + "`/agents:list`" + `; board ` + "`/agents:status`" + `; next move ` + "`/agents:next`" + `.
` + ClaudeEnd + "\n"
...
// /agents:list, /agents:status, /agents:next, /agents:new) — the same commands
